...
// 通用的 JSON 格式说明函数
function getSpeechFormatInstruction(role: string): string {
  let roleSpecificTip = '';
...
${getSpeechFormatInstruction('villager')}`;
}
...
${getSpeechFormatInstruction('werewolf')}`;
}
...
- 需要保护的玩家: 暂无
${getSpeechFormatInstruction('seer')}`;
}
...
当前局势分析：
- 今晚被杀的玩家: ${killedInfo}（你${context.witchHealUsed ? '已救' : '未救'}）
- 是否使用毒药: ${context.witchPoisonUsed ? '已使用' : '未使用'}
- 可疑玩家: ${params.suspiciousPlayers?.join('、') || '暂无明确目标'}
${getSpeechFormatInstruction('witch')}`;
}


// 工厂函数
export function getRoleSpeech(playerServer: PlayerServer, context: PlayerContext): string {
