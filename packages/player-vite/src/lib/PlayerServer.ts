...
export class PlayerServer {
  ...
  constructor(config: PlayerConfig) {
    console.log("Player server init");
    this.config = config;
  }
  ...
  async startGame(params: StartGameParams): Promise<void> {
    ...
    console.log("startGame:role:",this,this.role);
  ...
  async speak(context: PlayerContext): Promise<string> {
    console.log('🎭 Debug speak - role:', this.role);
    console.log('🔑 Debug speak - config.ai.apiKey exists:', !!this.config.ai.apiKey);
    console.log('🌍 Debug speak - env.OPENROUTER_API_KEY exists:', !!process.env.OPENROUTER_API_KEY);
    
    if (!this.role || (!this.config.ai.apiKey && !process.env.OPENROUTER_API_KEY)) {
      console.log('❌ Debug speak - returning fallback response');
      return "我需要仔细思考一下当前的情况。";
    }

    console.log('✅ Debug speak - proceeding with AI generation');
  ...
  async useAbility(context: PlayerContext | WitchContext | SeerContext): Promise<any> {
    console.log("useAbility:",this,this.role,this.config.ai,process.env.OPENROUTER_API_KEY);
  ...
  private getModel() {
    const openrouter = createOpenAICompatible({
      ...
      apiKey: process.env.OPENROUTER_API_KEY || this.config.ai.apiKey,
      headers: {
    ...
    const modelName = this.config.ai.model === 'custom' && (this.config.ai as any).customModel 
      ? (this.config.ai as any).customModel 
