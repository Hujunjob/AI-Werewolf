...
import { VotingResponseSchema } from '../../src/validation';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.log('\n=== SERVERLESS VOTE REQUEST ===');
    console.log('Player ID:', playerId);
    console.log('Context:', JSON.stringify(playerContext, null, 2));

    // 调用 vote 方法
    const voteResponse = await playerServer.vote(playerContext);

    // 验证响应格式
    const response = VotingResponseSchema.parse(voteResponse);
    
    console.log('Response:', JSON.stringify(response, null, 2));
    console.log('=== END SERVERLESS VOTE REQUEST ===\n');

    res.json(response);

  } catch (error) {
    console.error('Serverless vote error:', error);
